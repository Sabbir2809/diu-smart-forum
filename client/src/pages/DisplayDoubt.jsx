          ...
          <div className='section_i_outer'>
            <div className='section_i_wrapper'>
              <div className='left_hand_side'>
                ...
                <div className='display_doubt_title'>{requestedDoubt?.doubtData?.doubtTitle}</div>
              </div>
                ...
                <div className='votes_wrappper'>
                  <Button
                    ...
                    className={`custom_btn active ${
                      requestedDoubt?.doubtData?.upVotes?.indexOf(user?._id) === -1 ? 'black_dull' : 'black'
                    }`}>
                    <ArrowDropUpSharp style={{ scale: '1.5' }} />
                  </Button>
                  <div className='vote_count'>{requestedDoubt?.doubtData?.upVotes?.length}</div>
                  <Button
                    onClick={async () => {
                      await addVoteToDoubtFunction({
                        doubtData: requestedDoubt?.doubtData,
                    ...
                    className={`custom_btn active ${
                      requestedDoubt?.doubtData?.downVotes?.indexOf(user?._id) === -1 ? 'black_dull' : 'black'
                    }`}>
                ...
                <div className='doubt_main_wrapper'>
                  <div className='owner_info_outer'>
                    <div className='owner_info_wrapper'>
                      ...
                      <div
                        onClick={() => navigate(`/account?user=${requestedDoubt?.ownerInfo?.name}`)}
                      ...
                      {Math.floor(
                        Math.abs(Date.now() - Date.parse(requestedDoubt?.doubtData?.createdAt)) / (1000 * 60)
                      ) < 60 ? (
                        <div className='doubt_posted_time'>{`created ${Math.floor(
                          Math.abs(Date.now() - Date.parse(requestedDoubt?.doubtData?.createdAt)) /
                            (1000 * 60)
                        )} minutes ago`}</div>
                      ) : Math.floor(
                          Math.abs(Date.now() - Date.parse(requestedDoubt?.doubtData?.createdAt)) /
                  ...
                  <div className='doubt_tags_outer'>
                    <div className='doubt_tags_wrapper'>
                      {requestedDoubt?.doubtData?.tags?.map((tag, idx) => (
                        <Chip key={idx} label={tag} className={`home-post-tags active`} />
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <div className='section_iii_navigation'>
                <div className='left_hand_side'>
                  <Button
                    disabled
                    startIcon={<ChatIcon />}
                    className='custom_btn black doubt_action_btn'>{`Comments: ${
                    !requestedDoubt?.replies?.length ? 0 : requestedDoubt?.replies?.length
                  }`}</Button>
                </div>
                <div className='right_hand_side'>
                  <div className='btn_group'>
                      ...
                      <div className='single_reply_wrapper'>
                        <div className='owner_info_outer'>
                          <div className='owner_info_wrapper'>
                            ...
                            <div
                              onClick={() => navigate(`/account?user=${reply?.ownerInfo?.name}`)}
                              className='doubt_owner_name'>
                            ...
                            <div className='reply_action_outer'>
                              <IconButton
                                ...
                                <ArrowDropUpSharp
                                  className={`${
                                    reply?.replyData?.upVotes?.indexOf(user?._id) === -1
                                      ? 'black_dull'
                                      : 'black'
                                  }`}
                                />
                              ...
                              <div
                                ...
                                {!reply?.replyData?.upVotes?.length ? 0 : reply?.replyData?.upVotes?.length}
                              </div>
                              <IconButton
                                onClick={async () => {
                                  await addVoteToReplyFunction({
                                ...
                                <ArrowDropDownIcon
                                  className={`${
                                    reply?.replyData?.downVotes?.indexOf(user?._id) === -1
                                      ? 'black_dull'
                                      : 'black'
                                  }`}
                                />
                            ...
                            {Math.floor(
                              Math.abs(Date.now() - Date.parse(reply?.replyData?.createdAt)) / (1000 * 60)
                            ) < 60 ? (
                              <div className='doubt_posted_time'>
                                ...
                                Math.abs(Date.now() - Date.parse(reply?.replyData?.createdAt)) /
                                  (1000 * 60 * 60)
                              ) < 24 ? (
                              <div className='doubt_posted_time'>{`created ${Math.floor(
                                Math.abs(Date.now() - Date.parse(reply?.replyData?.createdAt)) /
                        ...
                        <div className='doubt_description_outer'>
                          <div className='doubt_description_wrapper'>{parse(reply?.replyData?.reply)}</div>
                        </div>
                      </div>
                    </div>
                  ))}
