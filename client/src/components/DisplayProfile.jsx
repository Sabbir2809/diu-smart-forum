...
const DisplayProfile = () => {
  ...
  const [linkedInUsername, setLinkedInUsername] = useState(queriedUser?.linkedInLink);
  ...
  return (
    <>
      {loading && (
        <CircularProgress
          style={{ width: 40, height: 40, position: 'absolute', top: '50%', left: '50%', color: '#1772cd' }}
        />
      )}
      <Snackbar onClose={() => setResponse(false)} autoHideDuration={2000} open={response}>
        <Alert variant='filled' severity={`${isError ? 'error' : 'success'}`} sx={{ width: '100%' }}>
          {alertMessage}
        </Alert>
      </Snackbar>
      {!loading && !!queriedUser && (
              ...
              <div className='user-profile-top-i-inner'>
                ...
                <div className='user-name-div'>
                  <div className='user-display-name'>
                    {queriedUser?.displayName?.[0]?.toUpperCase() + queriedUser?.displayName?.substr(1)}
                  </div>
                ...
                <div className='skill_previewer width_100'>
                  {!!queriedUser?.technicalSkills?.length &&
                    queriedUser?.technicalSkills?.map((skill, idx) => (
                      <Chip key={idx} className='home-post-tags active' label={skill} />
                    ))}
                </div>
              </div>
              <div className='community-stats'>
                ...
                <div className='stats-info'>
                  <Chip className='stats-chip' label={`${queriedUser?.materialCount} Material Uploaded`} />
                  <Chip className='stats-chip' label={`${queriedUser?.doubtsCount} Doubts Posted`} />
                  <Chip className='stats-chip' label={`${queriedUser?.repliesCount} Replied to Doubts`} />
                  <Chip
                    className='stats-chip'
                    label={`${
                      (queriedUser?.reputation > 0 ? '+' : '') + queriedUser?.reputation
                    } Reputation Earned`}
                  />
                </div>
              </div>
            ...
            <div className='lower-navigation'>
              <div className='btn-group'>
                <Button
                  ...
                  variant='text'
                  className={`custom_btn ${options === 'recent_material' ? 'active' : ''}`}
                ...
                <Button
                  ...
                  variant='text'
                  className={`custom_btn ${options === 'recent_doubts' ? 'active' : ''}`}
                  startIcon={<HelpOutlineRoundedIcon />}>
                  Recent Doubts
                </Button>
              </div>
              <div className='navigation_link'>
                <Button onClick={() => navigate('/')} className='custom_btn' endIcon={<SendRoundedIcon />}>
                  Go to Discuss
                </Button>
              </div>
            </div>
                ...
                <div className='lower_insider'>
                  {optionsResult.map((element, idx) => (
                    <div className='options_outer' key={idx}>
                      ...
                      {Math.floor(Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60)) < 60 ? (
                        <div className='posted_time'>{`${Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60)
                        )} minutes ago`}</div>
                      ) : Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60 * 60)
                        ) < 24 ? (
                        <div className='posted_time'>{`${Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60 * 60)
                        )} Hours ago`}</div>
                      ) : Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60 * 60 * 24)
                        ) < 30 ? (
                        <div className='posted_time'>{`${Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60 * 60 * 24)
                        )} Days ago`}</div>
                      ) : Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60 * 60 * 24 * 30)
                        ) < 12 ? (
                        <div className='posted_time'>{`${Math.floor(
                          Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60 * 60 * 24 * 30)
                        )} Months ago`}</div>
                  ...
                  {!!optionsResult &&
                    optionsResult?.map((element, idx) => (
                      <div
                        ...
                        onClick={() => navigate(`/doubt?id=${element?._id}&src=${window.location.pathname}`)}
                        key={idx}>
                        ...
                        <div className='doubt_statistics'>
                          ...
                          {Math.floor(Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60)) <
                          60 ? (
                            <div className='posted_time'>{`${Math.floor(
                              Math.abs(Date.now() - Date.parse(element.createdAt)) / (1000 * 60)
                            )} minutes ago`}</div>
