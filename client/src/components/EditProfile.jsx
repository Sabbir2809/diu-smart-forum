...
const EditProfile = () => {
  ...
  const handleUpdate = async () => {
    ...
    if (
      linkedInLink.length &&
      !(linkedInLink.includes('https://linkedin.com/') || linkedInLink?.includes('https://www.linkedin.com/'))
    ) {
    ...
    if (userImg)
      try {
        ...
        enqueueSnackbar('Error occurred while uploading image! Please try again!', {
          variant: 'error',
          autoHideDuration: 3000,
        });
        return;
      ...
      <div className='edit_profile_wrapper'>
        <div className='edit_profile_left_sider'>
          <div className='user_profile_img'>
            ...
            <input onChange={(e) => setUserImg(e.target.files[0])} type='file' hidden id='user_new_img' />
          </div>
            ...
            {!!skills.length &&
              skills?.map((skill) => (
                <BootstrapTooltip title='Click to remove' placement='right'>
                  <Chip
                    onClick={() => setSkills((state) => state?.filter((tag) => tag !== skill))}
          ...
          <div className='input-item'>
            <TextField
              ...
              onKeyUp={(e) => {
                if (e.key === 'Enter') {
                  if (tempSkill.length) setSkills((state) => [...state, tempSkill]);
                  setTempSkill('');
                } else if (
                  e.key !== 'Alt' &&
                  e.key !== 'Backspace' &&
                ...
                else if (e.key === 'Backspace') setTempSkill((state) => state.substring(0, state.length - 1));
              }}
            />
          </div>
        </div>
        <div className='editable_fields_outer'>
          <div className='editable_fields_wrapper'>
            ...
            <Button
              ...
              endIcon={disableSubmit && <CircularProgress style={{ width: 20, height: 20 }} />}>
              {status}
            </Button>
          </div>
        </div>
