          ...
          {previewFile !== null ? (
            <>
              <BootstrapTooltip title='Close Preview' position='bottom'>
                <CloseRounded onClick={() => setOpenPreview(false)} className='close-preview-icon' />
              </BootstrapTooltip>
              <object
            ...
            <div className='post-input-item flex-row'>
              ...
              <div className='btn-group'>
                ...
                <Button
                  ...
                  endIcon={
                    disableSubmit ? <CircularProgress style={{ width: 17, height: 17 }} /> : <SendRounded />
                  }
                  className='custom_btn'>
                  {status}
                </Button>
              ...
              <div className='post-doubts-tags-outer'>
                <div className='post-doubts-tags-wrapper'>
                  {tags?.map((tag) => (
                    <Chip
                      onClick={() => {
                        if (postTags.indexOf(tag) === -1) setPostTags((state) => [...state, tag]);
                        else setPostTags((state) => state.filter((all) => all !== tag));
                      }}
                      className={`home-post-tags ${postTags?.indexOf(tag) !== -1 ? 'active' : ''}`}
                      key={tag}
                      label={tag}
                    />
                  ))}
                ...
                {attachedFiles?.map((file, idx) => (
                  <div className='attached-file-viewer' key={idx}>
                    <div className='attached-file-type-viewer'>
                      {file?.type?.substr(file?.type?.length - 3, 3) === 'pdf' ? (
                        <PictureAsPdfRounded
                          onClick={() => {
                            setPreviewFile(file);
                            setOpenPreview(true);
                    ...
                    <div className='post-action-outer'>
                      <div className='attached-file-name-viewer'>{file?.name}</div>
                      <div className='remove-action'>
                        <BootstrapTooltip title='Remove file' placement='right'>
                          <IconButton
                            onClick={() => {
                              document.getElementById('attach_file_input').value = null;
                              setAttachedFiles((state) => state?.filter((item, idx2) => idx2 !== idx));
                            }}>
                            <CloseRounded />
                          </IconButton>
                        </BootstrapTooltip>
